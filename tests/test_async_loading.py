...
async def test_async_load_with_options(test_data_dir: Path) -> None:
    ...
    file_path = test_data_dir / "test_4d_TXYC_50x512x512x3.tiff"
    assert file_path.exists(), f"File not found: {file_path}"
    options = configure_load_options(t_range=(10, 30), y_range=(100, 300), x_range=(200, 400))
...
async def test_async_load_multiple_files_concurrently(test_data_dir: Path) -> None:
    ...
    file_paths = [
        test_data_dir / "test_2d_XY_512x512.tiff",
        test_data_dir / "test_3d_TXY_50x512x512.h5",
        test_data_dir / "test_4d_TXYC_50x512x512x3.zarr",
        test_data_dir / "test_5d_TZCYX_50x50x512x512x3.tiff",
    ]
